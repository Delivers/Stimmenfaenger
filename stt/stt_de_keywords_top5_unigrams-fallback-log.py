#!/usr/bin/env python3
# stt_de_keywords_top5_unigrams-fallback-log.py
# Erweiterung: Wenn 60s kein Mikrofoninput -> zufällige Zitate aus Datei senden (alle 30s),
# bis wieder Sprache erkannt wird.
# NEU: Jeder echte Live-Input (nicht Zitat) wird mit Zeitstempel in live_inputs_log.txt geschrieben.
...
def log_live_input(utterance: str, log_path: Path):
    """Speichert den erkannten Live-Satz in eine Logdatei mit Zeitstempel."""
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with log_path.open("a", encoding="utf-8") as f:
        f.write(f"[{ts}] {utterance}\n")

def process_utterance(utt, min_len, is_live=False, log_path: Path | None = None):
    utt = (utt or "").strip()
    if len(utt) < min_len:
        return False
    ...
    for k, s in top5:
        ...
        send_udp(k, utt, s)
    print("", flush=True)
    # Nur Live-Inputs loggen (nicht Zitate)
    if is_live and log_path:
        log_live_input(utt, log_path)
    return True
...
# ---------------------------------------------------
# Hauptschleife mit Fallback & Logging
# ---------------------------------------------------

def stream_with_fallback(model, samplerate, device, min_utt_chars,
                         inactivity_seconds, fallback_interval, quotes, log_path):
        ...
        try:
            while True:
                ...
                if in_fallback and now >= next_quote_time:
                    ...
                    process_utterance(quote, min_len=1, is_live=False)
                ...
                if rec.AcceptWaveform(data):
                    ...
                    if process_utterance(utt, min_utt_chars, is_live=True, log_path=log_path):
                        last_activity = time.monotonic()
                        if in_fallback:
                            in_fallback = False
...
def main():
    ap = argparse.ArgumentParser(description="Deutsches STT mit automatischem Zitat-Fallback & Logging.")
    ...
    stream_with_fallback(model, args.samplerate, args.device,
                         ...
                         args.fallback_interval, quotes, log_path)
